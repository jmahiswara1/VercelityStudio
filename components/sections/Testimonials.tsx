...
export default function Testimonials() {
    ...
    const scroll = (direction: 'left' | 'right') => {
        if (sliderRef.current) {
            // Lebar satu card kira-kira 400px + gap 24px = 424px
        ...
        >
            <Container>
                ...
                <motion.div
                    ...
                    ref={sliderRef}
                    className="flex overflow-x-auto gap-6 snap-x snap-mandatory hide-scrollbar pb-8 cursor-grab active:cursor-grabbing w-full px-2"
                    style={{ scrollbarWidth: 'none', msOverflowStyle: 'none' }} // Hide scrollbar for Firefox and IE
                >
                    {/* Hide scrollbar for webkit (Chrome, Safari) but kept in JSX via global styles normally, wait we should add a raw style if not in globals */}
                    ...
                    {testimonialsData.map((review, index) => (
                        <div
                            key={review.id}
                            className="min-w-[85vw] md:min-w-[420px] max-w-[420px] bg-[#f4f5f7] rounded-[24px] p-8 md:p-10 snap-center flex flex-col border border-black/5 shrink-0"
                        >
                            {/* Card Header (Avatar & X Icon) */}
                            <div className="flex justify-between items-start mb-6">
                                <div className="w-12 h-12 rounded-full overflow-hidden relative border border-black/10">
                                    <Image
                                        src={review.avatar}
                                        alt={review.author}
                                        fill
                            ...
                            {/* Review Text */}
                            <p className="text-black/80 font-medium leading-relaxed mb-10 flex-1">
                                {review.text}
                            </p>
